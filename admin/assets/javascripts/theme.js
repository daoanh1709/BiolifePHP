...
function signInCheck() {
    ...
    $.ajax({
        type: 'get',
        url: 'http://localhost:1000/Biolife/admin/process/signin_process.php',
        data: {
            action: "login",
            email: email,
            pass: pass,
        ...
        success: function (response) {
            if (response == "Success") {
                window.location.href = "http://localhost:1000/Biolife/admin/homeadmin.php";
            } else {
...
function vieworder(id) {
    $.ajax({
        type: 'get',
        url: 'http://localhost:1000/Biolife/admin/process/vieworder_process.php',
        data: {
            action: "view",
            id: id
        },
        success: function (response) {
            document.getElementById('modalOrderDetails').innerHTML = response;
            $('#openViewOrder').click();
        }
    });
}
...
function updateorder(id) {
    window.location.href = "http://localhost:1000/Biolife/admin/orderupdate.php?id=" + id;
}
...
$(document).ready(function (e) {
    $("#formOrder").on('submit', (function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: "http://localhost:1000/Biolife/admin/process/order_process.php",
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function (response) {
                if (response == "Success") {
                    showModalSuccess("The order has been updated successfully.");
                    document.getElementById("submit").onclick = function () {
                        window.location.href = "http://localhost:1000/Biolife/admin/orderview.php";
                    };
                } else if (response == "cannot") {
                    showModalError("You cannot update this order status.");
                } else if (response == "unconfirmedtodelivering") {
                    showModalError("You cannot update from Unconfirmed to Delivering.");
                } else if (response == "unconfirmedtodelivered") {
                    showModalError("You cannot update from Unconfirmed to Delivered.");
                } else if (response == "confirmedtounconfirmed") {
                    showModalError("You cannot update from Confirmed to Unconfirmed.");
                } else if (response == "confirmedtodelivered") {
                    showModalError("You cannot update from Confirmed to Delivered.");
                }
...
var proID = "";
$('.productDealTable tr td').on('click', function () {
    var $row = $(this).closest('tr')
    proID = $row.attr('id');
    var proName = $row.find('span.proname').text();
    document.getElementById('dealProduct').value = proName;
});
...
$(document).ready(function (e) {
    $("#dealForm").on('submit', (function (e) {
        ...
        formData.append("dealProID", proID);
        if (proID == "") {
            showModalError("Please select a product in the Products table below.")
            valid = false;
        }
        var discountStr = document.getElementById('dealDiscount').value;
        ...
        if (valid) {
            $.ajax({
                ...
                processData: false,
                success: function (response) {
                    if (response == "FalseDate") {
                        showModalError("Start date must be earlier than End date");
                    } else if (response == "Success") {
                        showModalSuccess("The deal has been added successfully.");
                        document.getElementById("submit").onclick = function () {
                            window.location.href = "http://localhost:1000/Biolife/admin/dealview.php";
                        };
...
function checkProduct1() {
    var valid = true;
    var priceStr = document.getElementById('productPrice').value;
    var price = parseFloat(priceStr);
    if (isNaN(price)) {
        showModalError("Price must be a real number.");
        valid = false;
    } else {
        if (price < 0) {
            showModalError("Price must be greater than 0.");
            valid = false;
        }
    }
    return valid;
...
$(document).ready(function (e) {
    $("#formProduct").on('submit', (function (e) {
        ...
        if (checkProduct1() === true) {
            ...
            var details = $('textarea[name="DSC"]').val();
            formData.append("details", details);
            formData.append("action", action);
            if (valid) {
                $.ajax({
                    type: 'POST',
                    url: "http://localhost:1000/Biolife/admin/process/product_process.php",
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function (response) {
                        if (response == "UpdateSuccess") {
                            showModalSuccess("The product has been updated successfully.");
                            document.getElementById("submit").onclick = function () {
                                window.location.href = "http://localhost:1000/Biolife/admin/productview.php";
                            };
                        } else if (response == "Fail") {
                            showModalError("Failed.");
                        } else if (response == "Success") {
                            showModalSuccess("The product has been added successfully.");
                            document.getElementById("submit").onclick = function () {
                                window.location.href = "http://localhost:1000/Biolife/admin/productview.php";
                            };
                        }
...
$(document).ready(function (e) {
    $("#formProductInsert").on('submit', (function (e) {
        e.preventDefault();
        if (checkProduct1() === true) {
            var id = document.getElementById('productID').value;
            var action;
...
$(document).on("click", ".dialogConfirmProduct", function () {
    $.ajax({
        type: 'get',
        url: 'http://localhost:1000/Biolife/admin/process/product_process.php',
        data: {
        ...
        success: function (response) {
            if (response === "Success") {
                showModalSuccess("The product has been deleted successfully.");
            }
        }
    });
});

function feature_product(id, el) {
    var checked;
    if ($('#checkFeature').is(':checked')) {
        checked = 1;
    } else {
        checked = 0;
    }
...
$(document).on("click", ".dialogConfirmDeal", function () {
    $.ajax({
        type: 'get',
        url: 'http://localhost:1000/Biolife/admin/process/deal_process.php',
        data: {
        ...
        success: function (response) {
            if (response === "Success") {
                showModalSuccess("This deal has been deleted successfully.");
            }
        }
    });
});
...
function accountinfoCheck() {
    ...
    var mail = document.getElementById('profileEmail');
...
$(document).ready(function (e) {
    $("#formInfoAdmin").on('submit', (function (e) {
        e.preventDefault();
        if (accountinfoCheck() === true) {
            ...
            $.ajax({
                type: 'POST',
                url: "http://localhost:1000/Biolife/admin/process/admin_process.php",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function (response) {
                    alert(response);
                    $("#loginName").load(" #loginName");
                    $("#loginImage").load(" #loginImage");
                },
                error: function (data) {
                    alert("Fail");
                }
...
function changePassFormCheck() {
    var valid = true;
    var cur_pass = document.getElementById('profileCurrentPassword');
    var pass_pattern = /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/;
    var new_pass = document.getElementById('profileNewPassword');
    var cfpass = document.getElementById('profileConfirmPassword');
...
$(document).ready(function (e) {
    $("#changePassAdmin").on('submit', (function (e) {
        e.preventDefault();
