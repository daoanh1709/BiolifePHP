...
<html class="no-js" lang="en">
    ...
    <head>
        ...
        <title>Set Password | Biolife - Organic Food</title>
                        ...
                        <div class="col-lg-6 col-md-8 col-sm-8 col-xs-8" style="background-color: #ffffff; border-radius: 7px;box-shadow: 0 2px 4px rgb(0 0 0 / 10%), 0 8px 16px rgb(0 0 0 / 10%)">
                            <div class="signin-container">
                                ...
                                if ($_GET['key'] && $_GET['token']) {
                                    ...
                                    $email = $_GET['key'];
                                    $token = $_GET['token'];
                                    $linkReset = new LinkReset($email, $token, "");
                                    $result = $linkReset->checkEmailandToken($conn);
                                    $curDate = date("Y-m-d H:i:s");
                                    if ($result->num_rows > 0) {
                                        $row = $result->fetch_assoc();
                                        if ($row['exp_date'] >= $curDate) {
                                            ?>
                                            <form action="" name="frm-login" method="post" class="register-form" id="setPassForm">
                                                <input type="hidden" name="email" value="<?php echo $email; ?>">
                                                <input type="hidden" name="reset_link_token" value="<?php echo $token; ?>">
                                                <p class="form-row">
                                                    <label for="pass">New Password <span class="requite">*</span></label>
                                                    <input type="password" id="spass" name="spass" value="" class="txt-input" required>
                                                </p>
                                                <p class="form-row">
                                                    <label for="cfpassword">Confirm Password <span class="requite">*</span></label>
                                                    <input type="password" id="scfpassword" name="scfpassword" value="" class="txt-input" required>
                                                </p>
                                                <p class="form-row wrap-btn">
                                                    <button class="btn btn-submit btn-bold" type="submit" name="submitRes">submit</button><br><br>
                                                    <a href="index.php" class="link-to-help" style="vertical-align: unset; margin-left: 5px">Go to home page</a>
                                                </p>
                                            </form>
                                            <?php
                                        }
